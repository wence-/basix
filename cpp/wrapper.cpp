...
Each element has a `tabulate` function which returns the basis functions and a number of their derivatives, as desired.
  ...
  m.def("create_element", &libtab::create_element, docs["create_element"]);

  m.def("tabulate_polynomial_set", &polyset::tabulate, docs["tabulate_polnomial_set"]);

  m.def("compute_jacobi_deriv", &quadrature::compute_jacobi_deriv, docs["compute_jacobi_deriv"]);

  m.def("make_quadrature",
        py::overload_cast<const Eigen::ArrayXXd&, int>(
            &quadrature::make_quadrature), docs["make_quadrature"])
      .def("make_quadrature",
